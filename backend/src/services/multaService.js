...
class MultaService {
  ...
  static async crearOAgregarDetalle(fk_cliente, tipo, monto, descripcion = null, fecha = null, fk_prestamo = null, fk_libro = null) {
    ...
    try {
      ...
      const [clientes] = await conn.execute('SELECT id_cliente FROM cliente WHERE id_cliente = ?', [clienteId]);
      if (clientes.length === 0) {
        throw createValidationError('Cliente no encontrado', 404);
      }

      if (prestamoId) {
        const [prestamos] = await conn.execute('SELECT id_prestamo, fk_cliente FROM prestamos WHERE id_prestamo = ?', [prestamoId]);
      ...
      if (libroId) {
        const [libros] = await conn.execute('SELECT id_libro FROM libro WHERE id_libro = ?', [libroId]);
        if (libros.length === 0) {
  ...
  static async marcarPagada(id) {
    const [resultado] = await pool.execute('UPDATE multa SET estado = true WHERE id_multa = ?', [id]);
  ...
  static async eliminar(id) {
    const [multa] = await pool.execute('SELECT estado FROM multa WHERE id_multa = ?', [id]);
    if (multa.length === 0) {
      const error = new Error('Multa no encontrada');
      error.status = 404;
      throw error;
  ...
  static async procesarVencidos() {
    ...
    try {
      ...
      for (const prestamo of vencidos) {
        await conn.execute("UPDATE prestamos SET estado = 'Vencido' WHERE id_prestamo = ?", [prestamo.id_prestamo]);

        const monto = prestamo.dias_retraso * 1000;
        const descripcion = `Retraso de ${prestamo.dias_retraso} día(s) - Prestamo #${prestamo.id_prestamo}`;
      ...
      await conn.commit();
      console.log(`[CRON] ${vencidos.length} prestamo(s) vencido(s) procesados`);
    } catch (error) {
      await conn.rollback();
